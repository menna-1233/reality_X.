...
RealityX — Turning Real-World Problems into AI-Powered Solutions. بيخلي سكان أي كمباوند/جامعة/مدينة يبلّغوا عن أي مشكلة يشوفوها
...
src/
  ...
  pages/
    ...
    DashboardPage.tsx      # لوحة الإدارة (خريطة، إحصائيات، نشاط) — للإدارة بس
    FindingsPage.tsx       # إدارة البلاغات (كانبان/جدول) وتغيير الحالة — للإدارة بس
    AdminLoginPage.tsx      # تسجيل دخول الإدارة (إيميل + باسورد)
```

## المستخدمين: مواطن بدون تسجيل، إدارة بإيميل وباسورد

- **المواطن** يبلّغ ويشوف الفيد من غير أي تسجيل دخول خالص — `/`, `/feed`,
  `/reports/:id` صفحات عامة تمامًا.
- **الإدارة** بس هي اللي بتسجل دخول (`/admin/login` عبر Supabase Auth)
  عشان توصل لـ `/dashboard` و `/findings` (محميين بـ `RequireAdmin`)، وعشان
  تقدر تغيّر حالة أي بلاغ. التفاصيل والتحقق الفعلي في
